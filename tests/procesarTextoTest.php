...
require_once __DIR__.'/../backend/ProcesadorTexto.php';

class ProcesadorTextoTest extends TestCase
{
  private $sw = ['de','la','y','el'];

  public function test_quitarTildes()
  {
...
public function test_limpiar(): void
{
  ...
  {
    $p = new ProcesadorTexto($this->sw);
    $this->assertSame(['casa','arbol'], $p->filtrar(['de','casa','la','árbol']));
  }

  public function test_contar()
  {
    $p = new ProcesadorTexto($this->sw);
    $this->assertSame(['hola'=>2,'mundo'=>1], $p->contar(['hola','mundo','hola']));
  }

  /** @depends test_quitarTildes */
  public function test_pipelineCompleto()
  {
    $p = new ProcesadorTexto($this->sw);
